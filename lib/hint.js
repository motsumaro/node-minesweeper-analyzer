...
var Hint = /** @class */ (function () {
    ...
    // areaが等しいか
    Hint.areaEqual = function (area1, area2) {
        var l = area1.length;
        if (area2.length !== l)
            return false;
        for (var i = 0; i < l; i++)
    ...
    // area1がarea2の部分集合か
    Hint.areaInclude = function (area1, area2) {
    ...
    Hint.solve = function (hint1, hint2) {
        ...
        if (iHint.equals(hint1)) {
            if (iHint.min === iHint.max || hint2.breadth === iBreadth)
                hint2removable = true;
        }
        else if (iHint.equals(hint2)) {
            if (iHint.min === iHint.max || hint1.breadth === iBreadth)
                hint1removable = true;
        }
        else {
            newHints.push(iHint);
